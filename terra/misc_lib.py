...
def arguments(data=False, start=0, finish=False, separator=' ', sequence=False):
    ...
    if data == '': return '' if not sequence else ['']
    data = data.split(separator)
    if len(data) <= start: return '' if not sequence else ['']
...
def clean_files(dirs=['.', './terra', './terra/rules', './terra/sys_extensions', './storage', './extensions', './dAmnViper', './reflex', './rules']):
